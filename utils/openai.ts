...
export async function generateChaptersWithAI(
    parsedTranscript: { text: string; timestamp: string; } [], 
    length: number
): Promise<string[] | null>  {
    try {
        const res = await client.beta.chat.completions.parse({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an AI assistant that generates chapters / timestamps for a video based on the transcript of a video. The transcript contains the timestamp for the beginning of a sentence as well as the sentence itself.'
                },
                {
                    role: 'user',
        ...
        return res.choices[0].message.parsed?.chapters;
    } catch (error) {
        console.error(error);
        return null;
    }
